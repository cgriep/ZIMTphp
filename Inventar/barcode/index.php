...
<? if ($_REQUEST["p_submit"] == "CONVERT") {?>

<?
	if (!isset($_REQUEST["p_text"])){
		$p_textEnc = "1234567890";
	} else {
		$p_textEnc = rawurlencode($_REQUEST["p_text"]);
	}

	if (!isset($_REQUEST["p_xDim"])){
		$p_xDim = 1;
	}
         else
           $p_xDim = $_REQUEST["p_xDim"];
	if (!isset($_REQUEST["p_w2n"])){
		$p_w2n = 3;
	}
         else
           $p_w2n = $_REQUEST["p_w2n"];
	if (!isset($_REQUEST["p_charHeight"])){
		$p_charHeight = 50;
	}
         else
	...
	$msg="";
	if (!safeStr($_REQUEST["p_bcType"], $p_text)){
		$msg = "Non-Valid characters inside string";
	}

	$dest = "wrapper.php?p_bcType=".$_REQUEST["p_bcType"]."&p_text=$p_textEnc" .
		"&p_xDim=$p_xDim&p_w2n=$p_w2n&p_charGap=$p_charGap".
                 "&p_invert=".$_REQUEST["p_invert"]."&p_charHeight=$p_charHeight" .
		"&p_type=".$_REQUEST["p_type"]."&p_label=".$_REQUEST["p_label"].
                 "&p_rotAngle=".$_REQUEST["p_rotAngle"]."&p_checkDigit=".
                 $_REQUEST["p_checkDigit"];
         ?>
